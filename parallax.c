...
void Tt_Parallax_Update(Tt_Parallax *parallax)
{
	...
	if (!rainInitialized) {
		for (int i = 0; i < MAX_RAINDROPS; i++) {
			...
			drops[i].speed = 600 + rand() % 300;
			drops[i].width = 0.8f + (rand() % 3) * 0.1f;
			drops[i].color = nvgRGBAf(0.6f, 0.8f, 1.0f, 0.3f + (rand() % 5) * 0.10f);
		}
		rainInitialized = 1;
	}

    for (int i = 0; i < MAX_RAINDROPS; i++) {
        drops[i].x -= gDeltaTime * 100;
        ...
        nvgLineTo(gVg, drops[i].x - 8, drops[i].y + drops[i].length);
